...
<head>
<meta charset="UTF-8">
<title>10 JSTL and EL</title>
</head>
<body>
	<h3>JSTL 태그 라이브러리 테스트</h3>
	<p>
		기존에 사용하던 jsp의 스크립트릿, 출력식 기호를 대체합니다.<br>
		이유는 요청처리하는 자바코드와 화면을 구성하는 html 태그를 분리할 때<br>
		jsp 파일을 태그형식으로만 작성하기 위함입니다.
	</p>
	<p>
		태그 라이브러리는 맨 위 지시자에서 정의하고 사용합니다. <br>
		EL 기호 $ { }는 애트리뷰트 값만 출력할 수 있습니다.
	</p>
	<hr>
	<% String name = "홍길동"; %>
	<!-- 저장된 애트리뷰트 값을 출력합니다. -->
	...
	<!-- 4. if ~ else 형식 -->
	<h5>c:choose 테스트</h5>
	<c:set var="age" value="13" /> <!-- 애트리뷰트 값 변경하기 -->
	<h5>
		age: <c:out value="${age}" />
	</h5>
	<c:choose>
	...
	<ul>
		<!--
			java에서는 객체변수명.getXXX() getter 메소드를 사용합니다.
			하지만 EL은 알아서 getter 동작을 해줍니다.
			객체의 속성 이름을 정확히 작성하기.
		-->
		<li>customerId: ${customer.customId}</li>
